...
def handle_scaling(age, df):
    ...
    # cols_to_scale in the saved scaler use a different casing (e.g. 'Number_Of_Dependants')
    # than our dataframe (e.g. 'number_of_dependants') -> map by lowercasing before lookup
    actual_cols = [c if c in df.columns else c.lower() for c in cols_to_scale]

    df[actual_cols] = scaler.transform(df[actual_cols].values)
...
def align_to_model_columns(df, model):
    """Renames/reorders df's columns to match model.feature_names_in_ exactly,
    matching case-insensitively so 'age' lines up with 'Age', etc.
    Any expected column not found in df is filled with 0."""
    expected_cols = getattr(model, "feature_names_in_", None)
    if expected_cols is None:
        return df

    name_map = {c.lower(): c for c in expected_cols}
    rename_dict = {col: name_map[col.lower()] for col in df.columns if col.lower() in name_map}
